...
def get_files_from_filestore(job, files, work_dir, cache=True, docker=False):
    ...
    for name in files.keys():
        filepath = job.fileStore.readGlobalFile(files[name], '/'.join([work_dir, name]), cache=cache)
        outpath = filepath
        # First check if the file is gzipped
        _, ext = os.path.splitext(outpath)
        if ext in {'.tgz', '.gz'} and is_gzipfile(outpath):
            outpath = strip_xext(outpath)
            if ext == '.tgz':
                outpath += '.tar'
            with gzip.open(filepath, 'rb') as gz_in, open(outpath, 'w') as ungz_out:
                shutil.copyfileobj(gz_in, ungz_out)

        basename, ext = os.path.splitext(outpath)
        if ext == '.tar' and tarfile.is_tarfile(outpath):
            untargz(outpath, work_dir)
            outpath = basename
