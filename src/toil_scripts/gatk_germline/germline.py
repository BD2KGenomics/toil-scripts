...
def download_and_run_sample(job, uuid, url, config, url2=None, rg_line=None):
    ...
    See the germline config file for necessary parameters.
...
def gatk_germline_pipeline(job, uuid, url, url2, config, rg_line=None):
    """
...
def download_shared_files(job, config):
    ...
    shared_files = {'genome_fasta'}
    unrequired_files = {'genome_fai', 'genome_dict'}
    shared_files |= unrequired_files
    # Corrects common naming convention in other TOIL scripts
    ...
    if config.run_bwa:
        ...
        unrequired_files.add('alt')
        shared_files |= bwa_references
    if config.preprocess:
        preprocess_references = {'mills', 'phase', 'dbsnp'}
        shared_files |= preprocess_references
    if config.run_vqsr:
        vqsr_references = {'phase', 'mills', 'dbsnp', 'hapmap', 'omni'}
        shared_files |= vqsr_references
    ...
    for name in shared_files:
        try:
            ...
            if getattr(config, name, None) is None and name not in unrequired_files:
...
def setup_and_run_bwa_kit(job, url, url2, rg_line, config):
    """
    Downloads and aligns FASTQ or BAM files to reference genome using BWA.
    ...
    # Uses first url to deduce paired fastq file URL, if url looks like a paired file.
    if url2 is None and '1.fq' in url:
        url2 = url.replace('1.fq', '2.fq')
        job.fileStore.logToMaster(
            'Trying to find paired read using FASTQ URL:\n%s\n%s' % (url, fq2_url)
        )
...
def parse_manifest(path_to_manifest):
    ...
    :rtype: list[list]
    """
    bam_re = r"^(?P<uuid>\w+)\w(?P<url>.+\.[bsc][r]*am)"
    ...
    with open(path_to_manifest, 'r') as f:
        for line in f.readlines():
            ...
            require(urlparse(url).scheme and urlparse(url) or url.startswith('syn'),
                    'Invalid URL passed for {}'.format(url))
            samples.append((uuid, url, url2, rg_line))
    return samples


def configure_space_resource(attribute, default):
    """
    Converts input parameter from human readable bytes to raw integer value.
    ...
    if attribute is None:
        ...
        raise ValueError('Could not configure attribute: %s' % attribute)
...
def main():
    ...
    # Pipeline execution
        ...
        if not options.maxCores:
            options.maxCores = sys.maxint
        ...
        if getattr(inputs, 'preprocess_only', None) is None:
        ...
        # Arbitrary defaults
        inputs['xmx'] = configure_space_resource(inputs['xmx'], '10G')
        inputs['file_size'] = configure_space_resource(inputs['file_size'], '10G')
        inputs['cores'] = inputs.get('cores', 8)

        inputs['sorted'] = inputs.get('sorted', None)
