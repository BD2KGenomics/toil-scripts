...
def docker_call(work_dir, tool_parameters, tool, file_path='debug'):
    """
...
def start(job, shared_ids, input_args, sample):
    ...
    if input_args['ssec'] is None:
        ids['bam'] = job.addChildJobFn(download_from_url, url, uuid).rv()
    else:
        pass
...
def index(job, ids, input_args):
    ...
    #job.addChildJobFn(haplotype_caller, ids, input_args, cores=int(input_args['cpu_count']), memory='10 G', disk='15 G')
...
#         job.addChildJobFn(apply_vqsr_snp, ids, input_args, cores=int(input_args['cpu_count']), memory='10 G', disk='15 G')
#     else:
...
# def apply_vqsr_indel(job, ids, input_args, sample):
#     if not debug:
#         work_dir = job.fileStore.getLocalTempDir()
#         output_dir = input_args['output_dir']
#         mkdir_p(output_dir)
